...
Inductive cminsel_effstep (ge:genv):  (block -> Z -> bool) ->
  ...
  | cminsel_effstep_skip_seq: forall f s k sp e m,
      cminsel_effstep ge EmptyEffect (CMinSel_State f Sskip (Kseq s k) sp e) m
         (CMinSel_State f s k sp e) m

  | cminsel_effstep_skip_block: forall f k sp e m,
      cminsel_effstep ge EmptyEffect (CMinSel_State f Sskip (Kblock k) sp e) m
  ...
  | cminsel_effstep_store: forall f chunk addr al b k sp e m vl v vaddr m'
      ...
      cminsel_effstep ge (StoreEffect vaddr (encode_val chunk v))
  ...
  | cminsel_effstep_seq: forall f s1 s2 k sp e m,
      cminsel_effstep ge EmptyEffect (CMinSel_State f (Sseq s1 s2) k sp e) m
  ...
  | cminsel_effstep_block: forall f s k sp e m,
      cminsel_effstep ge EmptyEffect (CMinSel_State f (Sblock s) k sp e) m
         (CMinSel_State f s (Kblock k) sp e) m

  | cminsel_effstep_exit_seq: forall f n s k sp e m,
      cminsel_effstep ge EmptyEffect (CMinSel_State f (Sexit n) (Kseq s k) sp e) m
         (CMinSel_State f (Sexit n) k sp e) m

  | cminsel_effstep_exit_block_0: forall f k sp e m,
      cminsel_effstep ge EmptyEffect
         (CMinSel_State f (Sexit O) (Kblock k) sp e) m
         (CMinSel_State f Sskip k sp e) m
  | cminsel_effstep_exit_block_S: forall f n k sp e m,
      cminsel_effstep ge EmptyEffect
         (CMinSel_State f (Sexit (S n)) (Kblock k) sp e) m
         (CMinSel_State f (Sexit n) k sp e) m
  ...
  | cminsel_effstep_return_0: forall f k sp e m m',
      Mem.free m sp 0 f.(fn_stackspace) = Some m' ->
      cminsel_effstep ge (FreeEffect m 0 (f.(fn_stackspace)) sp)
         (CMinSel_State f (Sreturn None) k (Vptr sp Int.zero) e) m
...
Lemma cminselstep_effax1: forall (M : block -> Z -> bool) g c m c' m',
      ...
      (corestep (cminsel_memsem hf) g c m c' m' /\
       Mem.unchanged_on (fun (b : block) (ofs : Z) => M b ofs = false) m m').
Proof. 
intros.
  induction H.
  split. unfold corestep; simpl. econstructor.
  ...
  split. unfold corestep; simpl. econstructor; try eassumption. trivial.
         apply Mem.unchanged_on_refl.
  split. unfold corestep; simpl. econstructor; try eassumption. trivial.
         eapply FreeEffect_free; eassumption.
  split. unfold corestep; simpl. econstructor; eassumption.
...
Lemma cminsel_effstep_curWR: forall (M : block -> Z -> bool) g c m c' m',
      cminsel_effstep g M c m c' m' ->
...
Proof. intros. eapply Mem.perm_valid_block. eapply cminsel_effstep_curWR; eassumption. Qed.

Program Definition cminsel_eff_sem : 
  @EffectSem genv CMinSel_core.
Proof.
eapply Build_EffectSem with (sem := cminsel_memsem hf)(effstep:=cminsel_effstep).
apply cminselstep_effax1.
apply cminselstep_effax2.
apply cminsel_effstep_curWR.
Defined.
