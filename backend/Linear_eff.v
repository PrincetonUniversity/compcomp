...
Inductive linear_effstep: (block -> Z -> bool) -> Linear_core -> mem -> Linear_core -> mem -> Prop :=
  | lin_effexec_Lgetstack:
      ...
      linear_effstep EmptyEffect 
        (Linear_State s f sp (Lgetstack sl ofs ty dst :: b) rs rs0) m
        (Linear_State s f sp b rs' rs0) m
  | lin_effexec_Lsetstack:
      forall s f sp src sl ofs ty b rs m rs' rs0,
      rs' = Locmap.set (S sl ofs ty) (rs (R src)) (undef_regs (destroyed_by_setstack ty) rs) ->
      linear_effstep EmptyEffect 
        (Linear_State s f sp (Lsetstack src sl ofs ty :: b) rs rs0) m
        (Linear_State s f sp b rs' rs0) m
  | lin_effexec_Lop:
      ...
      linear_effstep EmptyEffect 
        (Linear_State s f sp (Lop op args res :: b) rs rs0) m
        (Linear_State s f sp b rs' rs0) m
  | lin_effexec_Lload:
      ...
      linear_effstep EmptyEffect 
        (Linear_State s f sp (Lload chunk addr args dst :: b) rs rs0) m
        (Linear_State s f sp b rs' rs0) m
  | lin_effexec_Lstore:
      ...
      linear_effstep (StoreEffect a (encode_val chunk (rs (R src))))
        (Linear_State s f sp (Lstore chunk addr args src :: b) rs rs0) m
        (Linear_State s f sp b rs' rs0) m'
  | lin_effexec_Lcall:
      ...
      sig = funsig f' ->
      linear_effstep EmptyEffect 
        (Linear_State s f sp (Lcall sig ros :: b) rs rs0) m
  ...
  | lin_effexec_Ltailcall:
      ...
      linear_effstep (FreeEffect m 0 (f.(fn_stacksize)) stk)
        (Linear_State s f (Vptr stk Int.zero) (Ltailcall sig ros :: b) rs rs0) m
  ...
  | lin_effexec_Lbuiltin:
      ...
      linear_effstep (BuiltinEffect ge ef (decode_longs (sig_args (ef_sig ef)) (reglist rs args)) m)
         (Linear_State s f sp (Lbuiltin ef args res :: b) rs rs0) m
         (Linear_State s f sp b rs' rs0) m'
(*NO ANNOTS YET
  | lin_effexec_Lannot:
      forall s f sp rs m ef args b t v m',
      external_call' ef ge (map rs args) m t v m' ->
      linear_effstep (Linear_State s f sp (Lannot ef args :: b) rs rs0) m
         (Linear_State s f sp b rs rs0) m'*)
  | lin_effexec_Llabel:
      forall s f sp lbl b rs m rs0,
      linear_effstep EmptyEffect 
        (Linear_State s f sp (Llabel lbl :: b) rs rs0) m
        (Linear_State s f sp b rs rs0) m
  | lin_effexec_Lgoto:
      forall s f sp lbl b rs m b' rs0,
      find_label lbl f.(fn_code) = Some b' ->
      linear_effstep EmptyEffect 
        (Linear_State s f sp (Lgoto lbl :: b) rs rs0) m
        (Linear_State s f sp b' rs rs0) m
  | lin_effexec_Lcond_true:
      ...
      find_label lbl f.(fn_code) = Some b' ->
      linear_effstep EmptyEffect 
        (Linear_State s f sp (Lcond cond args lbl :: b) rs rs0) m
        (Linear_State s f sp b' rs' rs0) m
  ...
  | lin_effexec_Ljumptable:
      ...
      linear_effstep EmptyEffect 
        (Linear_State s f sp (Ljumptable arg tbl :: b) rs rs0) m
        (Linear_State s f sp b' rs' rs0) m
  | lin_effexec_Lreturn:
      forall s f stk b rs m m' rs0,
      Mem.free m stk 0 f.(fn_stacksize) = Some m' ->
      linear_effstep (FreeEffect m 0 (f.(fn_stacksize)) stk)
        (Linear_State s f (Vptr stk Int.zero) (Lreturn :: b) rs rs0) m
...
(*NO RULE FOR EXTERNAL CALLS
  ...
  | lin_effexec_return:
      forall s f sp rs0 c retty rs m rs_init,
      linear_effstep EmptyEffect 
        (Linear_Returnstate (Stackframe f sp rs0 c :: s) retty rs rs_init) m
