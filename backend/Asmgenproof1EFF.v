...
Lemma agree_nextinstr_nf:
  forall j ms sp rs,
  agree j ms sp rs -> agree j ms sp (nextinstr_nf rs).
Proof.
...
Lemma mk_mov_correct_eff:
  ...
  mk_mov rd rs k = OK c ->
  exists rs2,
     eff_exec_straight ge fn EmptyEffect c rs1 m k rs2 m
  /\ rs2#rd = rs1#rs
...
Proof.
  ...
  assert (rs3#ECX = Vint x'). unfold rs3. Simplifs.
  exists rs5. split. 
  apply eff_exec_straight_step with rs2 m EmptyEffect. 
    simpl. rewrite A. simpl. rewrite Int.and_idem. auto. auto.
    eapply eff_exec_straight_step with rs3 m EmptyEffect.
      simpl. change (rs2 EAX) with (rs1 EAX). simpl in *. rewrite A. simpl. 
        rewrite (Int.add_commut Int.zero tnm1). rewrite Int.add_zero. auto. auto.
      apply eff_exec_straight_step with rs4 m EmptyEffect.
        simpl. rewrite Int.lt_sub_overflow. unfold rs4. destruct (Int.lt x Int.zero); simpl; auto.
        unfold rs4. destruct (Int.lt x Int.zero); simpl; auto.
        apply eff_exec_straight_one. auto. auto.
        reflexivity.
        reflexivity.
        reflexivity.
        reflexivity.
...
Lemma mk_intconv_correct_eff:
  ...
  (forall c rd rs r m,
   ...
   EmptyEffect = effect_instr ge c (mk rd rs) r m) ->
  exists rs2,
     eff_exec_straight ge fn EmptyEffect c rs1 m k rs2 m
  /\ rs2#rd = sem rs1#rs
...
Lemma mk_smallstore_correct_eff:
  ...
  forall (HEF: forall c r addr rs m, 
...
(* low reg *)
  monadInv H. econstructor; split. apply eff_exec_straight_one. rewrite H1. 
  unfold exec_store. rewrite H0. eauto. auto. reflexivity.
  intros; Simplifs.
(* high reg *)
...
(* EAX is mentioned. *)
  ...
  econstructor; split.
  apply eff_exec_straight_three with rs2 m1 rs3 m1.
  simpl. auto. 
  ...
  simpl. extensionality b. extensionality z. simpl.
  replace (Val.add (eval_addrmode ge addr rs1) (Vint Int.zero))
     with (eval_addrmode ge addr rs1).
  Focus 2. destruct (eval_addrmode ge addr rs1); simpl in H0; try discriminate.
           simpl. rewrite Int.add_zero; auto.
  remember (eval_addrmode ge addr rs1) as a.
    destruct a; inv H0.
    rewrite HEF. rewrite HEF. simpl. clear HEF.
    rewrite <- Heqa. simpl. 
    unfold StoreEffect. rewrite Int.add_zero.
    change (rs3 EAX) with (rs1 r).
    destruct (eq_block b0 b); try subst b0; simpl; trivial. rewrite e in *. 
    destruct (valid_block_dec m1 b). rewrite andb_true_r; trivial.
      elim n. apply Mem.store_valid_access_3 in H3. 
        eapply Mem.perm_valid_block. apply H3. instantiate (1:=Int.unsigned i).
...
(* EAX is not mentioned *)
  ...
  econstructor; split.
  apply eff_exec_straight_two with rs2 m1.
  simpl. auto.
  ...
  clear. intros. unfold rs2; Simplifs.
  auto. auto.

  simpl. extensionality b. extensionality z. simpl.
    rewrite HEF. rewrite HEF. simpl. clear HEF.
    change (rs2 EAX) with (rs1 r). remember (encode_val chunk (rs1 r)) as v; clear Heqv. 
    rewrite (addressing_mentions_correct addr EAX rs2 rs1); auto.
    remember (eval_addrmode ge addr rs1) as a.
    destruct a; inv H0.
    unfold StoreEffect. 
    destruct (eq_block b0 b); try subst b0; simpl; trivial. 
    destruct (valid_block_dec m1 b). rewrite andb_true_r; trivial.
      elim n. apply Mem.store_valid_access_3 in H2. 
        eapply Mem.perm_valid_block. apply H2. instantiate (1:=Int.unsigned i).
         specialize (size_chunk_pos chunk); intros; omega. 
    clear. intros. unfold rs2; Simplifs.
...
Lemma loadind_correct_eff:
  ...
  exists rs',
     eff_exec_straight ge fn EmptyEffect c rs m k rs' m
  /\ rs'#(preg_of dst) = v
...
Proof.
  ...
  split. apply eff_exec_straight_one. simpl. unfold exec_load. rewrite H1. rewrite H0.
  eauto. auto.
  simpl. reflexivity.
  intuition Simplifs.
  (* float *)
  exists (nextinstr_nf (rs#(preg_of dst) <- v)).
  split. destruct (preg_of dst); inv H; apply eff_exec_straight_one; simpl; auto.
...
Lemma storeind_correct_eff:
  ...
  exists rs',
     eff_exec_straight ge fn 
          (StoreEffect (Val.add rs#base (Vint ofs)) (encode_val (chunk_of_type ty) (rs#(preg_of src))))
          c rs m k rs' m'
...
Proof.
  ...
  split. apply eff_exec_straight_one. simpl. unfold exec_store. rewrite H1. rewrite H0.
  eauto. auto.
  simpl. rewrite Int.add_zero_l.
      rewrite (ireg_of_eq _ _ EQ). trivial.
  intros; Simplifs.
  (* float *)
  destruct (preg_of src); inv H.
  econstructor; split. apply eff_exec_straight_one.
  ...
  simpl. eauto. auto.
  simpl. rewrite Int.add_zero_l. 
      rewrite (freg_of_eq _ _ EQ). trivial.
...
Lemma mk_setcc_base_correct_eff:
  ...
  exists rs2,
  eff_exec_straight ge fn EmptyEffect (mk_setcc_base cond rd k) rs1 m k rs2 m
...
- (* base *)
  econstructor; split.
  apply eff_exec_straight_one. simpl; eauto. auto.
  reflexivity.
  split. Simplifs. intros; Simplifs. 
- (* or *)
  ...
  eapply eff_exec_straight_three.
  simpl; eauto.
...
Lemma mk_setcc_correct_eff:
  forall cond rd k rs1 m,
  exists rs2,
  eff_exec_straight ge fn EmptyEffect (mk_setcc cond rd k) rs1 m k rs2 m
  /\ rs2#rd = Val.of_optbool(eval_extcond cond rs1)
  /\ forall r, data_preg r = true -> r <> EAX /\ r <> ECX -> r <> rd -> rs2#r = rs1#r.
Proof.
  intros. unfold mk_setcc. destruct (low_ireg rd).
- apply mk_setcc_base_correct_eff.
- exploit mk_setcc_base_correct_eff. intros [rs2 [A [B C]]].
  econstructor; split. eapply eff_exec_straight_trans. eexact A. apply eff_exec_straight_one. 
...
Ltac TranslOpEff :=
  econstructor; split;
  [ apply eff_exec_straight_one; [ simpl; eauto | auto | reflexivity ]
  | split; [ Simplifs | intros; Simplifs ]].

Lemma transl_op_correct_eff:
...
Transparent destroyed_by_op.
  ...
  destruct op; simpl in TR; ArgsInv; simpl in EV; try (inv EV); try (apply SAME; TranslOpEff; fail).
(* move *)
  exploit mk_mov_correct_eff; eauto. intros [rs2 [A [B C]]]. 
  apply SAME. exists rs2. split. eauto. split. simpl. auto. intros. destruct H; auto.
(* intconst *)
  apply SAME. destruct (Int.eq_dec i Int.zero). subst i. TranslOpEff. TranslOpEff.
(* floatconst *)
  apply SAME. destruct (Float.eq_dec f Float.zero). subst f. TranslOpEff. TranslOpEff.
(* cast8signed *)
  apply SAME. eapply mk_intconv_correct_eff; eauto.
(* cast8unsigned *)
  apply SAME. eapply mk_intconv_correct_eff; eauto. 
(* cast16signed *)
  apply SAME. eapply mk_intconv_correct_eff; eauto.
(* cast16unsigned *)
  apply SAME. eapply mk_intconv_correct_eff; eauto.
(* mulhs *)
  apply SAME. TranslOpEff. destruct H1. Simplifs. 
(* mulhu *)
  apply SAME. TranslOpEff. destruct H1. Simplifs. 
(* div *)
  apply SAME.
  ...
  TranslOpEff. change (rs#EDX<-Vundef ECX) with (rs#ECX). rewrite H0; rewrite Heqo. eauto.
  auto. auto.
...
(* modu *)
  ...
  simpl in H3. destruct H3; Simplifs.
(* shrximm *)
  apply SAME. eapply mk_shrximm_correct_eff; eauto.
(* lea *)
  exploit transl_addressing_mode_correct; eauto. intros EA.
  TranslOpEff. rewrite nextinstr_inv; auto with asmgen. rewrite Pregmap.gss; auto.
(* intoffloat *)
  apply SAME. TranslOpEff. rewrite H0; auto.
(* floatofint *)
  apply SAME. TranslOpEff. rewrite H0; auto.
(* condition *)
  exploit transl_cond_correct_eff; eauto. intros [rs2 [P [Q R]]].
  exploit mk_setcc_correct_eff; eauto. intros [rs3 [S [T U]]].
  exists rs3.
  split. eapply eff_exec_straight_trans. eexact P. eexact S. reflexivity.
...
Proof.
  ...
  exists rs2. split. 
  destruct chunk; ArgsInv; apply eff_exec_straight_one; auto.
...
Lemma transl_store_correct_eff:
  ...
  exists rs',
     eff_exec_straight ge fn 
        (StoreEffect a (encode_val chunk(rs (preg_of src))))
        c rs m k rs' m'
...
(* int8signed *)
  exploit mk_smallstore_correct_eff; eauto. 
    intros. simpl. unfold exec_store.  
      destruct (eval_addrmode ge addr0 rs0); simpl; auto.
      rewrite Mem.store_signed_unsigned_8; auto.
    simpl; intros. destruct H as [rs2 [Eff DR]].
      exists rs2; split; trivial.
      rewrite EA' in Eff. clear EA'.
      assert (StoreEffect a (encode_val Mint8signed (rs x0))
            = StoreEffect a (encode_val Mint8unsigned (rs x0))).
          unfold StoreEffect. repeat rewrite encode_val_length. simpl. trivial.
      rewrite H; assumption. 
(* int8unsigned *)
  exploit mk_smallstore_correct_eff; eauto.
  simpl. destruct (eval_addrmode ge x rs); inv H1. trivial.
(* int16signed *)
  econstructor; split.
  apply eff_exec_straight_one. simpl. unfold exec_store. 
...
(* int16unsigned *)
  econstructor; split.
  apply eff_exec_straight_one. simpl. unfold exec_store. rewrite H1. eauto. auto.
  simpl. reflexivity.
  intros. Simplifs.
(* int32 *)
  econstructor; split.
...
(* float64 *)
  econstructor; split.
  apply eff_exec_straight_one. simpl. unfold exec_store. erewrite Mem.storev_float64al32; eauto. auto.
  simpl. reflexivity.
  intros. Simplifs.
(* float64al32 *)
  econstructor; split.
