...
exists st2' m2' mu',
   corestep (rtl_eff_sem hf) tge st2 m2 st2' m2'
  /\ intern_incr mu mu'
...
{ (* call *)
  ...
  intros TFD.
  eexists; eexists; exists mu; split.
    eapply rtl_corestep_exec_Icall with (fd := transf_fundef fd); eauto.
    apply sig_preserved.
  split. apply intern_incr_refl. 
  split. apply sm_inject_separated_same_sminj. 
  split. rewrite sm_locally_allocatedChar.
...
{ (* tailcall *)
  ...
  eexists; eexists; exists mu; split.
    eapply rtl_corestep_exec_Itailcall with (fd := transf_fundef fd); eauto.
    apply sig_preserved.
...
Lemma MATCH_initial: forall v 
  ...
  (R : list_norepet (List.map fst (prog_defs prog)))
...
Theorem transl_program_correct:
  forall (LNR: list_norepet (List.map fst (prog_defs prog)))
...
(*initial_core*)
  { intros. 
    exploit (MATCH_initial _ _ _ m1 j vals2 m2 DomS DomT H); eauto.
...
{ intros. exploit MATCH_corediagram; try eassumption.
  ...
  split; trivial.
  split; trivial.
  left. apply corestep_plus_one. trivial. }
(*effcorediagram*)
