...
Lemma agree_args_extern_incr f0 mu mu' args tys stack ls m2' sp' (WD' : SM_wd mu') :
  extern_incr mu mu' ->
  agree_args f0 mu args tys stack ls m2' sp' ->
  agree_args f0 mu' args tys stack ls m2' sp'.
Proof.
inversion 1; subst. constructor; auto; inv H2; auto. 
  destruct agree_args_inj0 as [args0 INJ]. exists args0.
    apply restrict_forall_vals_inject.
    apply forall_vals_inject_restrictD in INJ; auto. 
...
intros ? ? X. assert (X':=X).
apply (agree_sp_fresh0 b0 z).
destruct H1 as [? [ ? [ ? [ ? [ ? [ ? [ ? [ ?  ?]]]]]]]].
rewrite H5 in agree_sp_local0.
apply as_inj_locBlocks in X; eauto.
rewrite agree_sp_local0 in X.
destruct WD'.
destruct (disjoint_extern_local_Src b0); try (rewrite X in H10; discriminate).
rewrite <- X'.
unfold as_inj, join.

destruct (extern_of mu' b0) eqn:extof'; try destruct p.
apply extern_DomRng in extof'; destruct extof' as [HH HH'].
rewrite H10 in HH; discriminate.

destruct (extern_of mu b0) eqn:extof; try destruct p.
destruct H as [?[?[?[?[?[?[? [? [? ?]]]]]]]]].
apply H in extof; rewrite extof in extof'; discriminate.
rewrite H1; trivial.

destruct H1 as [_ [_ [_ [_ [<- _]]]]]; auto. 
Qed.
...
Qed.
  ...
  (*This lemma should be famous.*)
  Lemma extern_incr_vis_equal: 
    forall mu mu', 
      extern_incr mu mu' ->
      SM_wd mu -> SM_wd mu' ->
      restrict (as_inj mu) (vis mu) = restrict (as_inj mu') (vis mu').            
    intros.
    extensionality b1.
    destruct (restrict (as_inj mu') (vis mu') b1) eqn:H3.
    { destruct p.
      unfold restrict, vis in *.
      destruct H as [? [? [? [? [? [? [? [? [? ?]]]]]]]]].
      rewrite H6.
      rewrite H10.
      destruct ( locBlocksSrc mu' b1) eqn:loc; simpl in *.
      unfold as_inj, join in *.
      rewrite H2.
      rewrite locBlocksSrc_externNone; try (rewrite H11; assumption); auto.
      rewrite locBlocksSrc_externNone in H3; try (assumption).
    rewrite H6; auto. 

    destruct (frgnBlocksSrc mu' b1) eqn:frgn; try discriminate.
  unfold as_inj, join in *.
  destruct (extern_of mu b1) eqn:extof. 
  destruct p.
  apply H in extof. rewrite extof in H3; auto.
  rewrite <- H10 in frgn.
  apply H0 in frgn. destruct frgn as [b2' [z' [extof' frgn]]].
  rewrite extof' in extof. discriminate. }
    
    { unfold restrict, vis in *.
      destruct H as [? [? [? [? [? [? [? [? [? ?]]]]]]]]].
      rewrite H6.
      rewrite H10.
      destruct (locBlocksSrc mu' b1 || frgnBlocksSrc mu' b1); auto.
      unfold as_inj, join in *.
      destruct (extern_of mu b1) eqn: extof. destruct p. apply H in extof.
      rewrite extof in H3; discriminate.
      destruct (extern_of mu' b1); try destruct p; try discriminate.
      rewrite <- H3. rewrite H2; auto.      
    }
  Qed.

Lemma match_stacks_change_meminj_extern:
  forall mu mu' m m' sp0 ls0 m1',
  extern_incr mu mu' ->
  forall cs cs' sg bound bound',
  match_stacks mu m m' sp0 ls0 cs cs' sg bound bound' ->
  Ple bound' (Mem.nextblock m1') -> SM_wd mu -> SM_wd mu' ->
...
Proof.
  induction 2; intros.
  apply match_stacks_empty with hi; auto.
  erewrite <- extern_incr_vis_equal; eauto.
...
Proof. intros.
  destruct MC as [[MS [INJ [RC [PG [Glob [SMV WD]]]]]] MRR].
...
exists c2,
  initial_core (Mach_eff_sem hf return_address_offset) tge v vals2 = Some c2 /\
  MATCH c1
    (initial_SM DomS DomT
       (REACH m1
...
Proof. intros.
  ...
  inv H0. simpl. inv H3. rewrite <-H. eapply match_states_init; eauto. simpl.
  ...
  rewrite andb_true_iff in H2. destruct H2 as [H2 H6]. revert H2.
  ...
  intros H2 H8; apply agree_args_match_init; auto.
  simpl; rewrite H; auto. solve[rewrite <-Heqtys; auto].
  ...
  solve[rewrite val_has_type_list_func_charact, H; auto].
...
Proof. intros. 
destruct MTCH as [[MC [INJ [RC [PG [GF [SMV WD]]]]]] [MRR1 MRR2]].
  split; trivial. 
  split; trivial. 
  split; trivial. 
...
intros WDnu. split; auto. intros; subst.
split.
{ split. 2: split; assumption.
  clear MRR1 MRR2; split; auto.
...
Proof. intros.
simpl. 
 destruct MatchMu as [[MC [INJ [RC [PG [GFP [VAL WDmu]]]]]] MRR].
...
{ (*MATCH'*)
  ...
  - econstructor. 
    ...
    { (*wtlocset*) eapply wt_setlist_result; auto. }
    ...
    { (*agree_callee_save*) apply agree_callee_save_set_result; auto. }
...
Lemma genv_next_symbol_exists' b (ge0 : Genv.t Linear.fundef unit) l :
  list_norepet fst ## l -> 
  (Plt b (Genv.genv_next ge0) ->
    exists id, ~List.In id fst ## l /\ Genv.find_symbol ge0 id = Some b) -> 
...
Lemma genv_next_symbol_exists b :
  list_norepet fst ## (prog_defs prog) -> 
...
(*  split. rewrite alloc_right_sm_as_inj; intros ? ? ? -> ?; congruence.
  ...
  assert (Hple1: Ple (Genv.genv_next ge) (Mem.nextblock m)).
  { destruct PG as [XX [Y Z]].
    destruct SMV as [YY ZZ].
    unfold Ple. rewrite <-Pos.leb_le.
    destruct (Pos.leb (Genv.genv_next ge) (Mem.nextblock m)) eqn:?; auto.
    rewrite Pos.leb_nle in Heqb.
    assert (Heqb': (Genv.genv_next ge > Mem.nextblock m)%positive) by xomega.
    assert (exists b0, Psucc b0 = Genv.genv_next ge).
    { destruct (Genv.genv_next ge). 
      exists ((b~1)-1)%positive. simpl. auto.
      exists (Pos.pred (b~0))%positive. rewrite Pos.succ_pred. auto. xomega.
      xomega. }
    destruct H as [b0 H].
    ...
    destruct H as [id H].
    apply XX in H.
    apply as_inj_DomRng in H.
    destruct H as [H H2].
    specialize (YY _ H).
    unfold Mem.valid_block in YY. clear - Heqb' YY H'. xomega.
    auto.
    auto. }

  assert (Hple2: Ple (Genv.genv_next ge) (Mem.nextblock m2)).
  { destruct PG as [XX [Y Z]].
    destruct SMV as [YY ZZ].
    unfold Ple. rewrite <-Pos.leb_le.
    ...
    destruct H as [H H2].
    specialize (ZZ _ H2).
    unfold Mem.valid_block in ZZ. clear - Heqb' ZZ H'. xomega.
    auto.
    auto. }

  apply match_stacks_empty with (hi := (Genv.genv_next ge)); auto.
  clear - ALLOC STORE Hple1 Hple2.
  ...
  xomega.
    ...
    clear - LNR Glob PGR WD. 
    destruct PGR. unfold vis; intuition. destruct H0 as [H0 H2].
    eapply match_globalenvs_intro.
    * intros b Hplt. apply genv_next_symbol_exists in Hplt; auto.
      destruct Hplt as [id Hplt].
      apply (H _ _ Hplt).
    * intros.
      erewrite H2; eauto.
    * intros.
      apply (Genv.genv_symb_range _ _ H1).
    * intros.
      apply (Genv.genv_funs_range _ _ H1).
    * intros.
      unfold Genv.find_var_info in H1.
      apply (Genv.genv_vars_range _ _ H1). }
...
{ (* internal function *)
  ...
  { (*MATCH*)
    ...
    { subst. cut (Mem.valid_block m2 sp0). intros VAL CONTRA. subst.
      unfold Mem.valid_block in VAL. xomega. 
    ...
    econstructor; eauto. 
      ...
      rewrite dec_eq_false; auto. intros N; subst; xomega. 
...
(*halted*)
  { intros. destruct H as [[MC [RC [PG [Glob [VAL [WD INJ]]]]]] [MRR1 MRR2]].
