...
exists
  ...
  (let (y, d2) := cd' in
   ...
   exists
     ...
     match_core12 d1 mu1 st1' m1' c0 m0 /\ match_core23 d2 mu2 c0 m0 st2' m2' /\ no_junk mu1 m1' m0) /\
...
Proof.
  ...
  destruct (eff_diagram12 _ _ _ _ _ CS1 _ _ _ _ MC12)
    as [st2' [m2' [d12' [mu12' [njIncr12 [LocAlloc12
       [MC12' [U2 Y_MOD21]]]]]]]]; clear eff_diagram12.
  destruct Y_MOD21 as [Y MOD21].
 ...
 (*case1*) 
  destruct CS2.
  assert (FWD1: mem_forward m1 m1') by (eapply effstep_fwd; exact CS1).
  clear CS1.
  ...
  (*solve [eapply compose_sm_intern_separated; eauto]. *)
  ...
  split. exists st2', m2', mu12', mu23'.
     ...
     { (* no_junk *)
       move NJNK at bottom.
       unfold no_junk. unfold no_junk in NJNK.
       intros.
       unfold I_fiddle in IFDL.
       destruct InjIncr23 as [InjIncr23' [ext_eq23 ?]].
       rewrite <- ext_eq23 in H0.
       assert (mperm2: Mem.perm m2 b2 ofs Max Nonempty ) by 
           admit. (* The rational here: coresteps should preserve permissions in external blocks (notice b2 is external). This is tricky because "external for who?"... External is a property of the injection, not the memory. *)
       specialize (IFDL _ _ _ H0 _ mperm2); destruct IFDL as [b1 [delta12 [image12 mperm]]].
       exists b1, delta12; split.
       unfold as_inj, join; unfold as_inj, join in image12.
       destruct (extern_of mu12 b1) eqn: extof_b1.
       destruct InjIncr12 as [InjIncr12' [ext_eq12 ?]].
       rewrite ext_eq12 in extof_b1.
       rewrite extof_b1; simpl. destruct p; inversion image12; reflexivity.
       admit. (* should be a contradiction because b2 is external!!*)
