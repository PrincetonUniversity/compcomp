...
(* exploit Mem.alloc_result; eauto. intros RES.
  ...
  intros. destruct H1 as [f3 [HF1 [HF2 Hf3]]]. 
...
(*NEW: Refactoring of original proof:
  ...
  3. make place swhere meminj's are extended more explicit.
     This is done by exposing meminj's as additional parameter
     of match_state, so that individual lemmas can
     explicitly exisitenitally quantify over the extensions and 
     impose additional conditions (eg in case Builtin)
  Other steps one could do: 
   - refactoring of states into cores and memories
   - add knowlegsde the amounts to "at_external"*)

Inductive match_statesInj: meminj -> Csharpminor.state -> Cminor.state -> Prop :=
  | matchInj_state:
      forall fn s k e le m tfn ts tk sp te tm cenv xenv f lo hi cs sz
      (TRF: transl_funbody cenv sz fn = OK tfn)
      (TR: transl_stmt cenv xenv s = OK ts)
      ...
      (MCS: match_callstack f m tm
               ...
               (Mem.nextblock m) (Mem.nextblock tm))
      (MK: match_cont k tk cenv xenv cs),
      match_statesInj f (Csharpminor.State fn s k e le m)
                   (State tfn ts tk (Vptr sp Int.zero) te tm)
  | matchInj_state_seq:
      forall fn s1 s2 k e le m tfn ts1 tk sp te tm cenv xenv f lo hi cs sz
      (TRF: transl_funbody cenv sz fn = OK tfn)
      ...
      (MCS: match_callstack f m tm
               ...
               (Mem.nextblock m) (Mem.nextblock tm))
      (MK: match_cont (Csharpminor.Kseq s2 k) tk cenv xenv cs),
      match_statesInj f (Csharpminor.State fn (Csharpminor.Sseq s1 s2) k e le m)
                   (State tfn ts1 tk (Vptr sp Int.zero) te tm)
  | matchInj_callstate:
      forall fd args k m tfd targs tk tm f cs cenv
      (TR: transl_fundef fd = OK tfd)
      ...
      (ISCC: Csharpminor.is_call_cont k)
      (ARGSINJ: val_list_inject f args targs),
      match_statesInj f (Csharpminor.Callstate fd args k m)
                   (Callstate tfd targs tk tm)
  | matchInj_returnstate:
      forall v k m tv tk tm f cs cenv
      (MINJ: Mem.inject f m tm)
      (MCS: match_callstack f m tm cs (Mem.nextblock m) (Mem.nextblock tm))
      (MK: match_cont k tk cenv nil cs)
      (RESINJ: val_inject f v tv),
      match_statesInj f (Csharpminor.Returnstate v k m)
                   (Returnstate tv tk tm).

Lemma matchInj_Intro: forall S T, 
  match_states S T -> exists j, match_statesInj j S T.
Proof. intros. 
  induction H; eexists.
     eapply matchInj_state with (f:=f); eauto.
     eapply matchInj_state_seq with (f:=f); eauto.
     eapply matchInj_callstate with (f:=f); eauto.
     eapply matchInj_returnstate with (f:=f); eauto.
Qed.

Lemma matchInj_Elim: forall j S T, 
  match_statesInj j S T -> match_states S T.
Proof. intros. 
  induction H.
     eapply match_state with (f:=f); eauto.
     eapply match_state_seq with (f:=f); eauto.
     eapply match_callstate with (f:=f); eauto.
     eapply match_returnstate with (f:=f); eauto.
Qed.

Lemma transl_step_case_SkipSeq: 
forall cenv sz f tfn j m tm  e le te sp lo hi cs s k tk xenv
(TR: transl_funbody cenv sz f = OK tfn) 
(MINJ : Mem.inject j m tm) 
(MCS : match_callstack j m tm (Frame cenv tfn e le te sp lo hi :: cs)
        (Mem.nextblock m) (Mem.nextblock tm))
(MK : match_cont (Csharpminor.Kseq s k) tk cenv xenv cs),
exists T2 : state,
  plus step tge (State tfn Sskip tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.State f s k e le m) T2.
Proof. intros.
...
Lemma transl_step_case_SkipBlock: 
...
exists T2 : state,
  plus step tge (State tfn Sskip tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.State f Csharpminor.Sskip k e le m) T2.
Proof. intros.
  dependent induction MK.
...
Lemma transl_step_case_SkipCall:
...
exists T2 : state,
  plus step tge (State tfn Sskip tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.Returnstate Vundef k m') T2.
Proof. intros.
...
Lemma transl_step_case_Assign:
forall cenv sz f tfn j m tm e lenv te sp lo hi cs k tk xenv x a x0 v id
(EE: Csharpminor.eval_expr ge e lenv m a v)
(TRF : transl_funbody cenv sz f = OK tfn)
(MINJ : Mem.inject j m tm)
(MCS : match_callstack j m tm (Frame cenv tfn e lenv te sp lo hi :: cs)
        (Mem.nextblock m) (Mem.nextblock tm))
(MK : match_cont k tk cenv xenv cs)
(EQ : transl_expr cenv a = OK (x, x0)),
exists T2 : state,
  plus step tge (State tfn (Sassign id x) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.State f Csharpminor.Sskip k e (PTree.set id v lenv) m) T2.
Proof. intros.
...
*)
  econstructor; split.
  apply plus_one. econstructor. eassumption. (* eexact EXEC.*)
inv MCS.
  econstructor; eauto.
econstructor; eauto.
...
(*
Lemma transl_step_case_Set:
forall cenv sz f tfn j m tm e lenv te sp m' lo hi cs k tk xenv x x0 v a chunk addr vaddr
(EE1 : Csharpminor.eval_expr ge e lenv m addr vaddr)
(EE2 : Csharpminor.eval_expr ge e lenv m a v)
(STV : Mem.storev chunk m vaddr v = Some m')
(TRF : transl_funbody cenv sz f = OK tfn)
(MINJ : Mem.inject j m tm)
(MCS : match_callstack j m tm (Frame cenv tfn e lenv te sp lo hi :: cs)
        (Mem.nextblock m) (Mem.nextblock tm))
...
exists T2 : state,
  plus step tge
    (State tfn (make_store chunk x x1) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_states (Csharpminor.State f Csharpminor.Sskip k e lenv m') T2.

CASE has disappeared in CompCert 2.0
*)

Lemma transl_step_case_Store:
...
exists T2 : state,
  ...
  match_statesInj j (Csharpminor.State f Csharpminor.Sskip k e le m') T2.
Proof. intros.
  exploit transl_expr_correct. eauto. eauto. eexact EvAddr. eauto. 
  intros [tv1 [EVAL1 [VINJ1 APP1]]].
  exploit transl_expr_correct. eauto. eauto. eexact EvA. eauto. 
...
exists T2 : state,
  plus step tge
    (State tfn
       (Scall optid (Csharpminor.funsig fd) x x1) tk
       (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j
    (Csharpminor.Callstate fd vargs (Csharpminor.Kcall optid f e le k) m) T2.
Proof. intros.
...
(*This is the first case where the meminj increases  -the proof also guarantees additional properties, 
in addition to the inject-incr, like unchange_on etc. We'll need those!*)
Lemma transl_step_case_Builtin:
...
(ExtCall: external_call ef ge vargs m t vres m')
(TRF : transl_funbody cenv sz f = OK tfn)
(MINJ : Mem.inject j m tm)
(MCS : match_callstack j m tm (Frame cenv tfn e le te sp lo hi :: cs)
        (Mem.nextblock m) (Mem.nextblock tm))
...
exists T2 : state,
  plus step tge
    (State tfn (Sbuiltin optid ef x) tk
       (Vptr sp Int.zero) te tm) t T2 /\
exists j', inject_incr j j' /\
  match_statesInj j'
    (Csharpminor.State f Csharpminor.Sskip k e (set_optvar optid vres le) m')
    T2.
Proof. intros.
  exploit transl_exprlist_correct; eauto.
  intros [tvargs [EVAL2 VINJ2]].
...
exists T2 : state,
  plus step tge (State tfn (Sifthenelse x x1 x2) tk (Vptr sp Int.zero) te tm)
    E0 T2 /\
  match_statesInj j (Csharpminor.State f (if b then s1 else s2) k e le m) T2.
Proof. intros.
  exploit transl_expr_correct; eauto. intros [tv [EVAL [VINJ APP]]].
  exists (State tfn (if b then x1 else x2) tk (Vptr sp Int.zero) te tm); split.
  apply plus_one. eapply step_ifthenelse; eauto. eapply bool_of_val_inject; eauto.
  econstructor; eauto. destruct b; auto.
Qed.

Lemma transl_step_case_Loop:
...
exists T2 : state,
  plus step tge (State tfn (Sloop x) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j
    (Csharpminor.State f s (Csharpminor.Kseq (Csharpminor.Sloop s) k) e le m)
    T2.
Proof. intros.
  econstructor; split.
  apply plus_one. constructor. 
...
exists T2 : state,
  plus step tge (State tfn (Sblock x) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.State f s (Csharpminor.Kblock k) e le m) T2.
Proof. intros.
  econstructor; split.
  apply plus_one. constructor. 
  econstructor; eauto.
...
Lemma transl_step_case_ExitSeq:
...
exists T2 : state,
  plus step tge
    (State tfn (Sexit (shift_exit xenv n)) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.State f (Csharpminor.Sexit n) k e le m) T2.
Proof. intros.
  dependent induction MK.
  econstructor; split. 
  apply plus_one. constructor.
...
Lemma transl_step_case_ExitBlockZero:
forall cenv sz f tfn j m tm e le te sp lo hi cs k tk xenv 
(TRF : transl_funbody cenv sz f = OK tfn)
(MINJ : Mem.inject j m tm)
(MCS : match_callstack j m tm (Frame cenv tfn e le te sp lo hi :: cs)
...
exists T2 : state,
  plus step tge
    (State tfn (Sexit (shift_exit xenv 0)) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.State f Csharpminor.Sskip k e le m) T2.
Proof. intros.
  dependent induction MK.
  econstructor; split.
...
Lemma transl_step_case_ExitBlockNonzero:
...
exists T2 : state,
  plus step tge
    (State tfn (Sexit (shift_exit xenv (S n))) tk (Vptr sp Int.zero) te tm)
    E0 T2 /\
  match_statesInj j (Csharpminor.State f (Csharpminor.Sexit n) k e le m) T2.
Proof. intros.
  dependent induction MK.
...
(*Version of the above lemma switch_match_states, for relation match_statesInj*)
Lemma switch_match_statesInj:
  forall fn k e le m tfn ts tk sp te tm cenv xenv j lo hi cs sz ls body tk'
    (TRF: transl_funbody cenv sz fn = OK tfn)
    (TR: transl_lblstmt cenv (switch_env ls xenv) ls body = OK ts)
    (MINJ: Mem.inject j m tm)
    (MCS: match_callstack j m tm
               (Frame cenv tfn e le te sp lo hi :: cs)
               (Mem.nextblock m) (Mem.nextblock tm))
    (MK: match_cont k tk cenv xenv cs)
    (TK: transl_lblstmt_cont cenv xenv ls tk tk'),
  exists S,
  plus step tge (State tfn (Sexit O) tk' (Vptr sp Int.zero) te tm) E0 S
  /\ match_statesInj j (Csharpminor.State fn (seq_of_lbl_stmt ls) k e le m) S.
Proof.
  intros. destruct ls; simpl.
  inv TK. econstructor; split. 
    eapply plus_left. constructor. apply star_one. constructor. traceEq.
    eapply matchInj_state; eauto. 
  inv TK. econstructor; split.
    eapply plus_left. constructor. apply star_one. constructor. traceEq.
...
(EQ0 : transl_lblstmt cenv (switch_env cases xenv) cases
      ...
      OK ts),
exists T2 : state,
  plus step tge (State tfn ts tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j
    (Csharpminor.State f (seq_of_lbl_stmt (select_switch n cases)) k e le m)
    T2.
Proof. intros.
  exploit transl_expr_correct; eauto. intros [tv [EVAL [VINJ APP]]].
  inv VINJ.
  ...
  exploit switch_match_statesInj; eauto. intros [T2 [F G]].
  exists T2; split.
...
Lemma transl_step_case_ReturnNone:
forall cenv sz f tfn j m tm e le te sp lo hi cs k tk xenv m'
...
(MK : match_cont k tk cenv xenv cs),
exists T2 : state,
  plus step tge (State tfn (Sreturn None) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.Returnstate Vundef (Csharpminor.call_cont k) m')
    T2.
Proof. intros.
...
Lemma transl_step_case_ReturnSome:
...
(EQ : transl_expr cenv a = OK (x, x0)),
exists T2 : state,
  plus step tge (State tfn (Sreturn (Some x)) tk (Vptr sp Int.zero) te tm) E0
    T2 /\
  match_statesInj j (Csharpminor.Returnstate v (Csharpminor.call_cont k) m') T2.
Proof. intros.
  exploit transl_expr_correct; eauto. intros [tv [EVAL [VINJ APP]]].
  exploit match_callstack_freelist; eauto. intros [tm' [A [B C]]].
  econstructor; split.
...
(EQ : transl_stmt cenv xenv s = OK x),
exists T2 : state,
  plus step tge (State tfn (Slabel lbl x) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.State f s k e le m) T2.
Proof. intros.
  econstructor; split.
  apply plus_one. constructor.
...
(MK : match_cont k tk cenv xenv cs),
exists T2 : state,
  plus step tge (State tfn (Sgoto lbl) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j (Csharpminor.State f s' k' e le m) T2.
Proof. intros.
...
Lemma transl_step_case_InternalCall:
...
(MINJ : Mem.inject j m tm)
(MCS : match_callstack j m tm cs (Mem.nextblock m) (Mem.nextblock tm))
(MK : match_cont k tk cenv nil cs)
(ISCC : Csharpminor.is_call_cont k)
(ARGSINJ : val_list_inject j vargs targs)
(EQ : transl_function f = OK x),
exists T2 : state,
  plus step tge (Callstate (Internal x) targs tk tm) E0 T2 /\
exists j', inject_incr j j' /\
  match_statesInj j'
    (Csharpminor.State f (Csharpminor.fn_body f) k e lenv m1) T2.
Proof. intros.
  ...
  intros [j' [MCS2 [MINJ2 IINCR]]].
  econstructor; split.
    eapply plus_left. constructor; simpl; eauto. 
    simpl. apply star_refl. (*eapply star_left. constructor.
...
(*
  ...
  intros [j' [tm2 [EXEC [MINJ2 [IINCR MCS2]]]]].
  econstructor; split.
    eapply plus_left. constructor; simpl; eauto.  
...
Lemma transl_step_case_ExternalCall:
forall ef vargs m t vres m' cenv cs tm j k tk targs
(ExtCall : external_call ef ge vargs m t vres m')
(MINJ : Mem.inject j m tm)
(MCS : match_callstack j m tm cs (Mem.nextblock m) (Mem.nextblock tm))
(MK : match_cont k tk cenv nil cs)
...
(ARGSINJ : val_list_inject j vargs targs),
exists T2 : state,
  plus step tge (Callstate (External ef) targs tk tm) t T2 /\
exists j', inject_incr j j' /\
  match_statesInj j' (Csharpminor.Returnstate vres k m') T2.
Proof. intros.
  exploit match_callstack_match_globalenvs; eauto. intros [hi MG].
  exploit external_call_mem_inject; eauto. 
  eapply inj_preserves_globals; eauto.
...
Lemma transl_step_case_Return:
forall j m tm cs f e le k tk cenv v tv optid
(MINJ : Mem.inject j m tm)
(MCS : match_callstack j m tm cs (Mem.nextblock m) (Mem.nextblock tm))
(MK :  match_cont (Csharpminor.Kcall optid f e le k) tk cenv nil cs)
(RESINJ : val_inject j v tv),
exists T2 : state,
  @plus genv state step tge (Returnstate tv tk tm) E0 T2 /\
  match_statesInj j
    (Csharpminor.State f Csharpminor.Sskip k e (set_optvar optid v le) m) T2.
Proof. intros.
  inv MK. simpl.
  econstructor; split.
  apply plus_one. econstructor; eauto. 
...
Lemma transl_step_correct_MATCH:
  forall S1 t S2, Csharpminor.step ge S1 t S2 ->
  forall T1, match_states S1 T1 ->
  match S1 with
     Csharpminor.State f (Csharpminor.Sseq s1 s2) k e le m
       => (exists T2, plus step tge T1 t T2 /\ match_states S2 T2)
           \/ (measure S2 < measure S1 /\ t = E0 /\ match_states S2 T1)%nat
   | _ => (exists T2, plus step tge T1 t T2 /\ match_states S2 T2)
   end.
Proof.
  induction 1; intros T1 MSTATE.
(* skip seq *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_SkipSeq _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ TRF MINJ MCS MK) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
(* skip block *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_SkipBlock _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ TRF MINJ MCS MK) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
(* skip call *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_SkipCall _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ H H0 TRF MINJ MCS MK) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
(* assign *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_Assign _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ id H TRF MINJ MCS MK EQ) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
(* set -- no such case
  inv MSTATE. monadInv TR. rename f0 into j. xx
  exploit transl_expr_correct; eauto. inv EQ1.
  destruct (transl_step_case_Set _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ id H TRF MINJ MCS MK EQ) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.*)
(* store *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_Store _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ H1 H H0 TRF MINJ MCS MK EQ EQ1) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
(* call *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_Call _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ optid _ _ H H0 H1 TRF MINJ MCS MK EQ EQ1) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
(* builtin *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_Builtin _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ optid _ _ _ _ H H0 TRF MINJ MCS MK EQ) as [T2 [Steps [j' [Inc MSI]]]].
   apply matchInj_Elim in MSI. eauto.
(* seq *)
  inv MSTATE. 
  (*Case 1*) 
...
(* ifthenelse *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_Ite _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ H H0 TRF MINJ MCS MK EQ EQ1 EQ0) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* loop *)
  inv MSTATE. monadInv TR.
  destruct (transl_step_case_Loop _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ TRF MINJ MCS MK EQ) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* block *)
  inv MSTATE. monadInv TR.
  destruct (transl_step_case_Block _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ TRF MINJ MCS MK EQ) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* exit seq *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_ExitSeq _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ n _ TRF MINJ MCS MK) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* exit block 0 *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_ExitBlockZero _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ TRF MINJ MCS MK) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* exit block n+1 *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_ExitBlockNonzero _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ n TRF MINJ MCS MK) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* switch *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_Switch _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ H TRF MINJ MCS MK EQ EQ0) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* return none *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_ReturnNone _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ H TRF MINJ MCS MK) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
 
(* return some *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_ReturnSome _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ m' v H H0 TRF MINJ MCS MK EQ) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* label *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_Label _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ lbl  x _ TRF MINJ MCS MK EQ) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* goto *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_Goto _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ lbl s' k' H TRF MINJ MCS MK) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.

(* internal call *)
  inv MSTATE. monadInv TR. rename f0 into j. 
  destruct (transl_step_case_InternalCall _ _ _ _ _ _ _ _ _ _ _ _ _ _ H H0 H1 H2 H3 MINJ MCS MK ISCC ARGSINJ EQ) as [T2 [Steps [j' [Inc MSI]]]].
   apply matchInj_Elim in MSI. eauto.
...
(* return *)
  ...
  destruct (transl_step_case_Return _ _ _ _ _ _ _ _ _ _ _ _  optid MINJ MCS MK RESINJ) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
Qed.
...
Proof.
 intros.
 assert (X:= transl_step_correct_MATCH _ _ _ H _ H0).
 destruct S1.
   destruct s; try (left; assumption).
   assumption.
 left; assumption.
 left; assumption.
Qed.
