...
Lemma eval_expr_fun ge:   forall { e le m a v v'},
...
Lemma eval_exprlist_fun ge:   forall {e le m a ty v v'},
    Clight.eval_exprlist ge e le m a ty v -> Clight.eval_exprlist ge e le m a ty v' -> v=v'.
Proof.
  induction a; intros; inv H; inv H0; f_equal.
  apply (eval_expr_fun ge H3) in H6. subst. congruence.
  eauto.
Qed.


Lemma eval_lvalue_fun ge:   forall {e le m a b b' z z'},
...
Lemma inv_find_symbol_fun F V (ge : Genv.t F V):
  forall {id id' b},
...
Lemma eventval_list_match_fun F V (ge : Genv.t F V):
  forall {a a' t v}, 
    Events.eventval_list_match ge a t v ->
    Events.eventval_list_match ge a' t v ->
    a=a'.
Proof.
 intros.
 ...
 f_equal. clear - H6 H.
 inv H; inv H6; auto.
 apply (inv_find_symbol_fun _ _ ge H0) in H3; subst; auto.
 eauto.
Qed.
...
Ltac fun_tac :=
  ...
  | H: eval_expr ?ge ?e ?le ?m ?A _,
    H': Clight.eval_expr ?ge ?e ?le ?m ?A _ |- _ =>
        apply (eval_expr_fun ge H) in H'; subst
  | H: Clight.eval_exprlist ?ge ?e ?le ?m ?A ?ty _,
    H': Clight.eval_exprlist ?ge ?e ?le ?m ?A ?ty _ |- _ =>
        apply (eval_exprlist_fun ge H) in H'; subst
  | H: Clight.eval_lvalue ?ge ?e ?le ?m ?A _ _,
    H': Clight.eval_lvalue ?ge ?e ?le ?m ?A _ _ |- _ =>
        apply (eval_lvalue_fun ge H) in H'; inv H'
  | H: assign_loc ?ty ?m ?b ?ofs ?v _,
    H': assign_loc ?ty ?m ?b ?ofs ?v _ |- _ =>
        apply (assign_loc_fun H) in H'; inv H'
  | H: Clight.deref_loc ?ty ?m ?b ?ofs ?t _,
    H': Clight.deref_loc ?ty ?m ?b ?ofs ?t _ |- _ =>
  ...
  | H: Clight.bind_parameters ?e ?m ?p ?vl _,
    H': Clight.bind_parameters ?e ?m ?p ?vl _ |- _ =>
...
intros.
rename H0 into STEP; 
  revert q2 m2 STEP; induction H; intros; 
    try solve
    [ inv STEP; simpl; auto; repeat fun_tac; auto;
      try (match goal with | H : _ \/ _ |- _ => destruct H end; congruence)].
{ inv STEP. fun_tac. rewrite H17 in H2; inv H2; rewrite H18 in H3; inv H3; fun_tac; auto.
  destruct H12; congruence. destruct H12; congruence. }
{ inv STEP. fun_tac. eapply external_call_determ in H14; eauto.
  assert (t = nil). { eapply not_observed_empty_trace; eauto. }
  subst t; destruct H14 as [H14 [H16 H17]]. inv H14; auto. subst; auto.
  destruct H10; congruence. destruct H10; congruence. }
{ inv STEP; auto. elim H6. }
{ destruct H; subst; inv STEP; auto. elim H6. }
{ inv STEP; auto. elim H6. }
{ inv STEP; auto;  try solve[elim H]. rewrite H0 in H9; inv H9; auto. }
{ inv STEP; auto; try solve[destruct H8; congruence]. fun_tac; inv H; auto. }
{ destruct H; subst; inv STEP; auto. elim H6. }
{ inv STEP. inv H; inv H6.
