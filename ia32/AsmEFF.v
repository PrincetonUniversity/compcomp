...
Inductive step: state -> trace -> state -> Prop :=
  | exec_step_internal:
      ...
      exec_instr (fn_code f) i rs m = Next rs' m' ->
      step (State sg rs m) E0 (State sg rs' m')
  | exec_step_builtin:
      ...
      step (State sg rs m) t (State sg rs' m')
  | exec_step_annot:
      forall b ofs f ef args rs m vargs t v m' sg,
      rs PC = Vptr b ofs ->
      Genv.find_funct_ptr ge b = Some (Internal f) ->
      find_instr (Int.unsigned ofs) (fn_code f) = Some (Pannot ef args) ->
      annot_arguments rs m args vargs ->
      external_call' ef ge vargs m t v m' ->
      step (State sg rs m) t
           (State sg (nextinstr rs) m')
  | exec_step_external:
      forall b ef args res rs m t rs' m' sg,
...
Inductive final_state: state -> int -> Prop :=
  | final_state_intro: forall rs m r sg,
      rs#PC = Vzero ->
      rs#EAX = Vint r ->
      final_state (State sg rs m) r.
